...
- **A green run can be a disabled run** — the reason all of the above exists. Quote test counts, not
  just "green": a full local run is `348 passed | 2 skipped` at 24 files (274 before E2 started; the
  ledger encoding, schema and append-only trigger added the rest). A run narrowed with `-t`, a file filter,
