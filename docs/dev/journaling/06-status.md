...
| ~~ADR-007~~ | **Entschieden 2026-07-31: eine Kette je Mandant.** Offen nur noch die Spalte — `ingestion_sources.id` empfohlen          | E2              |
...
| 2026-07-31      | **ADR-007 entschieden (Auftraggeber): eine Kette _je Mandant_.** Nicht eine globale mit Mandanten-Tag — RFC §15 nennt per-tenant für **Export und Löschung** sauberer, und genau das muss dieses Produkt dauernd leisten; ein Kettenauszug für einen Auditor würde bei einer globalen Kette zwangsläufig Absender, Empfänger und Zeitpunkte anderer Mandanten offenlegen. **Die frühere Entwurfsrichtung war ein Denkfehler und ist berichtigt:** „für v1 eine einzelne Kette" berief sich auf RFC §5.2, dessen „do not start there" aber **Shard**-Ketten zur Horizontalskalierung meint, nicht eine fachliche Partition je Mandant. Sieben Konsequenzen in ADR-007 festgehalten, davon drei nicht offensichtlich: die Kettenkennung **muss in den Genesis** (sonst sind zwei Ketten mit identischem erstem Ereignis hashgleich und ein Eintrag zwischen Mandanten verschiebbar), das **Ankern** wird teurer und braucht in E7/E8 die Wahl zwischen einem TSA-Zeitstempel je Mandant und einem Aggregat über alle Kettenköpfe, und **`verify` muss eine _fehlende_ Kette erkennen** — ein Zustand, den eine globale Kette nicht darstellen konnte. `02-architektur.md` §4, ADR-006 (Genesis) und E2s Tasks `JR-203`/`JR-204`/`JR-206`/`JR-208`/`JR-209` sind nachgezogen. **Offen und bewusst nicht selbst entschieden:** welche Spalte `chain_scope_id` ist — `ingestion_sources.id` (empfohlen, weil der Ledger stabiler sein muss als die Endpunktkonfiguration, die ihn füllt: `journaling_sources` ist `paused`-bar, seine `routing_address` regenerierbar, es hängt per `onDelete: cascade` an der Ingestion-Source, und das Schema erlaubt mehrere Endpunkte je Archiv) oder `journaling_sources.id`. **Umgebung:** der Auftraggeber hat **Docker Sandboxes** (`Docker.sbx` 0.37.1) installiert — das ist **keine** Docker Engine auf dem Host, aber innerhalb einer Sandbox laufen Docker **29.6.1** und Compose **v5.2.0**, und `docker-compose.yml` ist darüber fahrbar. Alle vier Dienste gestartet, vom Host aus über `sbx ports` belegt: PostgreSQL **17.10** mit `CREATEDB`, Valkey `AUTH`+`PING`, Meilisearch `/health` `200`; Tika läuft (Jetty auf `0.0.0.0:9998`), ist vom Host aber nicht erreichbar. **Drei gemessene Auflagen:** die Sandbox stoppt im Leerlauf (Container **und** Portfreigaben kehren beim nächsten Start von selbst zurück — ein `409 already published` belegt das), der Portforwarder überlebt die **parallele Integrationslast nicht** (Volllauf vom Host: 16 Fehlschläge, alle `read ECONNRESET` bzw. `write CONNECTION_CLOSED`; derselbe Commit gegen den Embedded-Cluster grün mit `274 passed                                                                                                                                                                                                                                                               | 2 skipped`; ein einzelner Connect durch den Forwarder ist dagegen 4 ms schnell), und `sbx` erzwingt im Sandbox-Netz eine **Default-Deny**-Netzpolicy (`403 Blocked by network policy`für`http://tika:9998`). **Empfehlung:** Postgres weiter aus dem Embedded-Cluster, `sbx`-Docker für Valkey/Meilisearch/Tika ab E4/E6. Die Alternative „ganze Suite _in_ der Sandbox" umgeht den Forwarder, ist aber nicht geprüft und stößt auf ein für Windows gebautes `node_modules` im Bind-Mount. Kein Code geändert. | Spaltenfrage klären, dann **`JR-203`** (ADR-006), dann die übrigen E2-Tasks |
