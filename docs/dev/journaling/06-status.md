...
**Letzte Aktualisierung:** 2026-07-31 (**E2 läuft: `JR-203`, `JR-201`, `JR-202`, `JR-204` und `JR-205`
sind erledigt — ADR-006 und ADR-009 entschieden, `packages/journaling` existiert, die kanonische Kodierung
trifft die Testvektoren, `journal_ledger` plus `deployment_identity` sind migriert und append-only
erzwungen**) · **Branch:** `claude/journaling-e2-ledger`, abgezweigt vom Integrationsbranch (E13 ist
zurückgemergt)
