...
**Letzte Aktualisierung:** 2026-07-31 (**E2 läuft: `JR-203`, `JR-201`, `JR-202` und `JR-204` sind erledigt
— ADR-006 entschieden, `packages/journaling` existiert, die kanonische Kodierung trifft die Testvektoren,
und `journal_ledger` plus `deployment_identity` sind migriert**) · **Branch:**
`claude/journaling-e2-ledger`, abgezweigt vom Integrationsbranch (E13 ist zurückgemergt)
