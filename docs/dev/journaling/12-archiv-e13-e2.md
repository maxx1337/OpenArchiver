...
## E2 — Ledger und Hash-Chain: Task-Protokolle `JR-2-01` bis `JR-2-09`

**Ausgegliedert am 2026-08-06 aus dem Kopf von `06-status.md` (Doku-Diät, inhaltlich unverändert).**
Diese Blöcke standen dort unter der "Letzte Aktualisierung"-Notiz und wurden von der Diät des
2026-08-03 übersehen — nur die formellen Abnahmerunden (oben in dieser Datei) waren damals bereits
ausgegliedert.
...
## E13 — IAM-Autorisierung härten: weitere Notizen aus derselben Kopf-Notiz

**Ausgegliedert am 2026-08-06 aus dem Kopf von `06-status.md` (Doku-Diät, inhaltlich unverändert).
Ergänzt die vier Abnahmerunden weiter oben in dieser Datei um die Notizen, die zwischen den Runden
entstanden sind** (E13-Abnahme-Zusammenfassung, `JR-13-17`-Kurzfassung, `JR-13-09`-Ablehnung mit
F25–F29, ADR-019 samt Nachtrag, ADR-017-Entscheidung).
...
## E13 — zwei weitere Notizen aus `07-session-handover.md`

**Ausgegliedert am 2026-08-06 aus `07-session-handover.md` (Doku-Diät, inhaltlich unverändert).**
Diese zwei Blöcke standen dort ohne eigene Überschrift am Ende des Umgebungsabschnitts: die
Lückenschließung von `JR-13-09` nach `JR-13-09a` (HTTP-400-Pfad, Betreiber-SQL auf PostgreSQL
17.10) und die `JR-13-16`/`JR-13-17`-Nachtragsnotiz.
...
## E13 — die beiden Task-Tabellen aus `06-status.md`

**Ausgegliedert am 2026-08-06 aus `06-status.md` (Doku-Diät, inhaltlich unverändert).** Die
Nacharbeits-Tabelle (`JR-13-13`-`JR-13-15`, die drei erneuten Abnahmerunden) und die Haupttabelle
(`JR-13-01`-`JR-13-09`) — beide bereits in größerer Tiefe an anderer Stelle in dieser Datei bzw. in
`13-archiv-sessionprotokoll-bis-e3.md` belegt, hier als vollständiger Verweis-Anker erhalten.
...
## E13 — die Vier-Runden-Zusammenfassung aus `06-status.md`

**Ausgegliedert am 2026-08-06 aus `06-status.md` (Doku-Diät, inhaltlich unverändert)** — die
Kurzfassung des Wegs durch die vier Abnahmerunden, deren volle Protokolle bereits weiter oben in
dieser Datei stehen.
