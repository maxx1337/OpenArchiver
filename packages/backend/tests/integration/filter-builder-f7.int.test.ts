 ...
 * (JR-13-01, epic E13). Classification: `ci`.
 *
 * ---------------------------------------------------------------------------------------------
 * The defect
 * ---------------------------------------------------------------------------------------------
 ...
 *
 * Required behaviour (`JR-13-02`): `null` is a **deny**. The unrestricted return is only legitimate
 ...
 * therefore RED until `JR-13-02` lands. That is the point -- see `JR-13-01`'s acceptance criterion.
 *
...
suiteRequiring('ci', 'FilterBuilder fail-closed -- FINDING F7 (JR-13-01)', postgresProbe, () => {
	const db = () => harness!.db;
	...
	it(redUntil('JR-13-02', 'a user with no role at all sees no ingestion source'), async () => {
		coverageNotice(
			'FINDING F7 regression (JR-13-01): FilterBuilder.create() must not return ' +
				'{ drizzleFilter: undefined } for a principal with no matching `can` rule. ' +
				'Expected RED until JR-13-02. A user with no role is reachable by deleting a role ' +
	...
	it(redUntil('JR-13-02', 'a cannot-only policy denies the rows it forbids (F7b)'), async () => {
		...
		// JR-13-02's acceptance criterion, verbatim: no row for dev@openarchiver.com.
		expect(
			visible,
	...
	it(
		redUntil('JR-13-02', 'the same denial holds for the search action (F7b, ADR-017 path)'),
		async () => {
	...
	/**
	 * ADR-017 (variant B, decided 2026-07-29) -- the semantics `JR-13-01` is required to demand:
	 ...
	 *     returns `null` and `FilterBuilder` calls that full access. Combined with the pre-JR-13-03
	 ...
	 *     gate.
	 *
	 * That `SearchService` asks for `'search'` after `JR-13-03` is asserted in
	...
	it('an unconditional grant still means every row -- the counter-check', async () => {
		// Green before and after. `JR-13-02` must not turn `FilterBuilder.ts:31` into a deny; if it
