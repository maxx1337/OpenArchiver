...
/**
 * JR-101 / JR-102 -- the reference example for the `integration` category.
 *
 ...
 * guaranteed teardown, two runs in parallel without interference -- is JR-104 and is not started
 * here.
 *
 ...
 *
 * Classification: `ci` -- once a Postgres service container exists (JR-105) this runs on every PR.
...
suiteRequiring('ci', 'Postgres reachability (integration harness smoke)', postgresProbe, () => {
	...
	it('can create and drop an isolated schema (precondition for JR-104)', async () => {
